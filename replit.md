...
- **Analytics & User Insights (Oct 2025)**: Comprehensive tracking system with `analyticsEvents` table tracking story creation, completion, page regeneration, shares, and views. Centralized analytics service tracks events non-blocking. Admin API provides completion rates and popular prompt analysis.
- **Social Sharing Features (Oct 2025)**: Share storybooks on Facebook, Twitter, WhatsApp with copy-link functionality. Public gallery (/gallery) for opt-in story sharing with pagination. Storybooks track isPublic status, shareCount, and viewCount. Open Graph meta tags for social previews. View tracking for public gallery analytics.
- **User Feedback & Rating System (Oct 2025)**: Star rating (1-5) with optional text feedback via RatingDialog component. `storyRatings` table with unique constraint per user/storybook prevents duplicates. Display average ratings and rating count in storybook viewer. Rating submission tracked in analytics.
- **Page Regeneration (Oct 2025)**: Individual page regeneration for storybook owners. Maintains character consistency and story continuity using mainCharacterDescription, defaultClothing, and storyArc. Confirmation dialog prevents accidental changes. Regeneration button on each page (hover to reveal) with loading states. Analytics tracking for regeneration events.
- **Enhanced Admin Analytics Dashboard (Oct 2025)**: Comprehensive analytics at /admin/analytics showing total revenue (digital/print breakdown), revenue trends (30-day charts), active users, completion rates, average ratings. Popular themes analysis via keyword extraction. User retention metrics with growth trends. Top-rated stories table. Uses Recharts for visualizations.
- **Dynamic Music & Sound Effects (Oct 2025)**: AI-powered mood-adaptive audio system with 8 mood categories (calm, adventure, mystery, happy, suspense, dramatic, joyful, whimsical). Features Gemini AI mood detection per page, Web Audio API with equal-power crossfading between mood transitions, separate volume controls for music and effects, storybook-scoped audio preferences (musicEnabled, soundEffectsEnabled, musicVolume, effectsVolume), and authenticated API endpoints requiring storybook ownership. Audio settings persist in `audioSettings` table with storybookId FK and unique constraint. Frontend uses AudioManager class with loadTrack(), crossfadeTo(), and playEffect() methods. Page-turn sound effects integrated into flipbook viewer.
